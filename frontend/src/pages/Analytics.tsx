  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        {/* Mobile controls: period pills scrollable + export */}
        <div className="flex md:hidden flex-col gap-2 w-full">
          {/* Period pill bar - horizontal scroll */}
          <div className="flex overflow-x-auto gap-1.5 -mx-4 px-4 pb-1">
            {PERIOD_OPTIONS.map((opt) => (
              <button
                key={opt.key}
                type="button"
          ...
          <div className="flex gap-2">
            <select
              value={agentFilter}
              onChange={(e) => setAgentFilter(e.target.value)}
              className="flex-1 px-3 h-10 border border-slate-200 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-500 text-slate-700"
            >
              <option>All Agents</option>
              <option>AI Only</option>
              <option>Human Only</option>
            ...
            <button
              type="button"
              className="flex-shrink-0 inline-flex items-center justify-center gap-1.5 px-3 h-10 border border-slate-200 rounded-lg text-sm font-medium text-slate-700 bg-white hover:bg-slate-50 transition-colors"
            >
              <Download size={14} />
            </button>
          </div>
      ...
      {/* Charts section - hidden on mobile, visible md+ */}
      <div className="hidden md:block">
        ...
        {/* Agent Performance - desktop table */}
      ...
      {/* Mobile-only data panels (shown instead of charts) */}
      <div className="md:hidden space-y-4">
        {/* Lead Sources - mobile list */}
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
          <h2 className="font-bold text-slate-900 text-sm mb-3">Lead Sources</h2>
          <div className="space-y-2.5">
            {leadSources.map((src) => (
              <div key={src.label}>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-sm font-bold" style={{ color: src.color }}>{src.pct}%</span>
                </div>
                <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                  <div
                    className="h-full rounded-full"
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
          <h2 className="font-bold text-slate-900 text-sm mb-3">Conversion Funnel</h2>
          <div className="space-y-2.5">
            {conversionFunnel.map((step) => (
              <div key={step.label} className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
                <div className="flex items-center gap-2">
        ...
        {/* AI Volume - mobile */}
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
          <h2 className="font-bold text-slate-900 text-sm mb-3">AI Volume</h2>
          <div className="space-y-2.5">
            <div className="flex items-center justify-between p-3 rounded-xl border" style={{ background: '#eff6ff', borderColor: '#bfdbfe' }}>
              <div>
                <p className="text-sm font-semibold text-slate-700">AI Handled</p>
                <p className="text-xs text-slate-500 mt-0.5">69.5% of total</p>
              </div>
              <p className="text-2xl font-extrabold text-brand-700">1,284</p>
            </div>
            <div className="flex items-center justify-between p-3 rounded-xl border" style={{ background: '#fffbeb', borderColor: '#fde68a' }}>
              <div>
                <p className="text-sm font-semibold text-slate-700">AI Handoffs</p>
                <p className="text-xs text-slate-500 mt-0.5">11.4% of AI convos</p>
              </div>
              <p className="text-2xl font-extrabold text-amber-700">147</p>
            </div>
            <div className="flex items-center justify-between p-3 rounded-xl border bg-[#f8fafc] border-slate-200">
              <div>
                <p className="text-sm font-semibold text-slate-700">Qualification Rate</p>
                <p className="text-xs text-slate-500 mt-0.5">vs human: +6.2%</p>
              </div>
              <p className="text-2xl font-extrabold text-slate-800">72.4%</p>
            </div>
          </div>
        </div>

        {/* AI vs Human - mobile list */}
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
          <h2 className="font-bold text-slate-900 text-sm mb-3">AI vs Human</h2>
          <div className="space-y-0 divide-y divide-slate-100">
            {aiVsHuman.map((row) => (
              <div key={row.metric} className="flex items-center justify-between py-3">
                <span className="text-sm text-slate-600 flex-1 pr-2">{row.metric}</span>
                <div className="flex items-center gap-3 flex-shrink-0">
                  <div className="text-center">
                    <p className="text-xs text-slate-400 mb-0.5">AI</p>
                    <p className="text-sm font-bold text-brand-700">{row.ai}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-xs text-slate-400 mb-0.5">Human</p>
                    <p className="text-sm font-medium text-slate-500">{row.human}</p>
                  </div>
                </div>
              </div>
            ))}
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">{agent.leads} leads · {agent.tours} tours · {agent.deals} deals</p>
                </div>
                <div className="flex-shrink-0 text-right">
                  <p className="text-sm font-bold" style={{ color: convColor }}>{agent.conv}%</p>
                  <p className="text-[10px] text-slate-400 mt-0.5">{agent.response}</p>
                </div>
              </div>
            );
          })}
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          {monthlyTrends.map((row, i) => (
            ...
            >
              <div>
                <p className="font-semibold text-sm text-slate-700">{row.month}</p>
                <p className="text-xs text-slate-400 mt-0.5">{row.leads.toLocaleString()} leads · {row.tours} tours · {row.deals} deals</p>
              </div>
              <span className="flex-shrink-0 ml-3 text-xs font-bold px-2.5 py-1 rounded-full" style={{ background: '#f0fdf4', color: '#16a34a' }}>
                {row.rate}
              </span>
            </div>
          ))}
